...
class NegativeBinomialModel(object):
    ...
    def build_model(self):
        ...
        cost1 = -T.mean(likelihoods) #+ T.mean(T.abs_(Y - mu))
        cost2 = -T.mean(likelihoods_) #+ T.mean(T.abs_(Y - mu_))
        cost3 = cost1 + cost2
        ...
        for grad_beta, beta in zip(grad_betas, self.betas + self.betas_ + self.betas_shared):
        ...
        outputs = []
        outputs.extend(self.betas + self.betas_ + self.betas_shared)
        outputs.extend(grad_betas)
        outputs.extend(self.alphas)
        outputs.extend(grad_alphas)
        outputs.extend([cost3, likelihoods, likelihoods_])
    ...
    def set_beta_value(self, betas, shape, dtype=floatX, epsilon=1e-2):
        beta_value = None
        for beta in betas:
    ...
    def train_model(self,
        ...
        while epoch < n_epochs and not stop_looping:
            ...
            if (len(costs) > 0 and (abs(costs[-1] - cost) < 1e-3)):
                    # or costs[-1] < outputs[-3])):
                break

            _n = 5
            betas.append(outputs[:_n])
            grad_betas.append(outputs[_n:_n*2])
            alphas.append(outputs[_n*2:_n*2+2])
            grad_alphas.append(outputs[_n*2+2:_n*2+4])
            costs.append(cost)
            likelihoods.append(outputs[-2:])
            # if len(grad_alphas) > 1:
            #     grad_flag = True  # if difference of all grads < 1e-6
            #     for grad_new, grad_old in zip(grad_betas[-1], grad_betas[-2]):
            #         if np.any(np.abs(grad_new-grad_old) > 1e-4):
            #             grad_flag = False
            #             break
        ...
        return betas, alphas, grad_betas, grad_alphas, costs, likelihoods
    ...
    def predict(self, betas, shape, conditions, condition):
        ...
        beta_i, beta_ic, beta_it, beta_i_, beta_s = betas[-1]
